		...
		public string DeviceName
		{
			...
			set
			{
				...
				for (int i = 0; i < WebCamTexture.devices.Length && -1 == cameraIndex; i++)
				{
					...
					if (WebCamTexture.devices[i].name != "OBS Virtual Camera" && WebCamTexture.devices[i].name != "Twitch Virtual Camera")
					{
						cameraIndex = i;
				...
				{
					cameraIndex = 0;
					webCamDevice = WebCamTexture.devices[cameraIndex]; //cameraindex by default
